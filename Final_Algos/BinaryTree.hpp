...
public:
	...
	void replace(const T& dest, const T& src, const function<bool(const T&, const T&)>& crit) {
		_replace(root, dest, src, crit);
	}
...
private:
	...
	void _replace(Node<T>* node, const T& dest, const T& src, const function<bool(const T&,const T&)>& crit) {
		if (node) {
			_replace(node->left, dest, src, crit);
			if (crit(dest, node->value)) {
				node->value = src;
			}
			_replace(node->right, dest, src, crit);
		}
	}
